...
public class VisualizeRay : MonoBehaviour
{
	...
	private void Start()
	{
		handController = object_grasped.GetComponent<HandController>();
	}
	...
	private void Update()
	{
		if (holding_object)
		{
			...
			if (moving_object)
			{
				...
				if (Vector3.Distance(target_location, object_grasped.transform.position) > 0.1f)
				{
					object_grasped.velocity = (target_location - object_grasped.transform.position).normalized * 8f;
				}
				else
				{
